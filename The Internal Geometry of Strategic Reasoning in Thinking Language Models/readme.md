...
- **Single annotator, calibrated on baseline text only.** GPT-5.4 only, no second judge. The 0.77 F1 was measured on normal (unsteered) Qwen outputs. Steered and ablated outputs, especially the degenerate high-alpha ones, are exactly the cases where annotation is least reliable, and I have no calibration there. Llama annotation quality is also unmeasured. Relative comparisons across conditions are more meaningful than absolute rates, but condition-dependent annotation bias remains possible.
- **Steering rates are confounded by generation length.** Interventions change how much text is produced (e.g. baseline 2,540 segments vs steer_-0.5 6,257, and 10,741 at OOD). Because label rates are per-segment, a shift in opp_mod rate can partly reflect changed generation dynamics rather than a clean behavioral shift. The ablate-vs-steer asymmetry (full-sequence projection vs last-token addition) compounds this. Within-condition relative comparisons are safer than cross-condition absolute rates.
- **Construct validity of the labels.** The whole pipeline assumes the GPT-5.4 label `opponent_modeling` carves a real behavioral joint. The probe result is relevant here: the DoM direction is not the model's own discriminant, so "opponent_modeling" may be a useful behavioral descriptor without being the category the model internally uses.
...
2. **Disentangle steering effects from generation length.** Length-matched / truncation-controlled comparisons, and reporting absolute opp_mod counts alongside per-segment rates, to check the steering shifts survive once generation dynamics are held fixed.
3. **A second annotator and calibration on steered/ablated outputs.** Calibration so far is baseline-text only; the modified outputs are where it matters most.
4. **Disambiguate the ablation result.** A norm-matched random steering control, position-matched ablation, and routing-measurement would separate routing-around from a regulatory direction.
5. **A judge-independent behavioral metric** sensitive enough to detect reasoning-quality change (GPT-5.4 is used for both vector extraction labels and steered-output evaluation, circularity risk).
